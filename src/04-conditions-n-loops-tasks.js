...
function findFirstSingleChar(str) {
  ...
  return str.split('').find((char) => count[char] === 1) || null;
}
...
function getCommonDirectoryPath(pathes) {
  ...
  if (common.length > 0 && result !== '' && !result.endsWith('/')) {
    return `${result}/`;
  }
  return result;
}
...
function evaluateTicTacToePosition(position) {
  ...
  return lines.reduce((winner, line) => {
    ...
    const [a, b, c] = line;
    const val1 = position[a[0]]?.[a[1]];
    const val2 = position[b[0]]?.[b[1]];
    const val3 = position[c[0]]?.[c[1]];
    return val1 && val1 === val2 && val1 === val3 ? val1 : winner;
  }, undefined);
}
